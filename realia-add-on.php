...
$realia_addon->import_images( 'realia_addon_property_images', 'Property Images' );
...
if ( realia_version() < 4 ) {
	...
	$realia_addon->import_images( 'realia_addon_property_plans', 'Floor Plans' );
...
function realia_import($post_id, $data, $import_options, $article) {
	...
	if ( realia_version() < 4 ) {
		...
		foreach( $fields as $field ) {
			...
			if ( empty( $article['ID'] ) or $realia_addon->can_update_meta( $field, $import_options ) ) {
	    ...
	    if ( empty( $article['ID'] ) or $realia_addon->can_update_image( $import_options ) ) {
		...
		if ( empty( $article['ID'] ) or ( $realia_addon->can_update_image( $import_options ) && $realia_addon->can_update_meta( $field, $import_options ) ) ) {
	    ...
	    if ( empty( $article['ID'] ) or ( $realia_addon->can_update_meta( $field, $import_options ) && $realia_addon->can_update_meta( '_property_latitude', $import_options ) && $realia_addon->can_update_meta( '_property_longitude', $import_options ) && !empty ( $search ) ) ) {
	    ...
	    foreach ( $fields as $key => $value ) {
	        ...
	        if ( empty( $article['ID'] ) or $realia_addon->can_update_meta( $key, $import_options ) ) {
		...
		if ( empty( $article['ID'] ) or $realia_addon->can_update_meta( $field, $import_options ) ) {
			...
			$title_ids = array_map( 'strval', $title_ids );
		...
		// if all fields are updateable and $search has a value
		if ( empty( $article['ID'] ) or ( $realia_addon->can_update_meta( 'property_map_location', $import_options ) && $realia_addon->can_update_meta( 'property_map_location_longitude', $import_options ) && $realia_addon->can_update_meta( 'property_map_location_latitude', $import_options ) && !empty ( $search ) ) ) {
...
add_action( 'pmxi_saved_post', 'realia_addon_update_post_meta_fields', 10, 1 );

function realia_addon_update_post_meta_fields( $post_id ) {
...
function realia_addon_property_images( $post_id, $attachment_id, $image_filepath, $import_options ) {
...
function realia_addon_property_plans( $post_id, $attachment_id, $image_filepath, $import_options ) {
